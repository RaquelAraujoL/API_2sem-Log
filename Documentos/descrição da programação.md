# Descrição da programação de limpeza e tratamento de dados através do Python/Pandas sobre a planilha de transportes de produtos perigosos/combustíveis.
...
Ação: Filtra linhas com "TONELADA". Multiplica por 1200.0 (fator de conversão de peso para volume estipulado pelo projeto) e muda para 'LITRO'.

Python
mask_grama = Dados_Padronizados['Unidade de Medida'].str.contains('GRAMA', na=False)
Dados_Padronizados.loc[mask_grama, 'Quantidade Transportada'] *= 0.00125
Dados_Padronizados.loc[mask_grama, 'Unidade de Medida'] = 'LITRO'
Ação: Filtra linhas com "GRAMA". Multiplica por 0.00125 para achar o volume correspondente e converte a unidade para 'LITRO'.

Python
Dados_Padronizados.head(10)
Ação: Mostra as primeiras 10 linhas da tabela para conferir se as conversões matemáticas e as mudanças de nomes de unidades funcionaram.
...
Ação: Salva a tabela final totalmente limpa, padronizada e enriquecida em um novo arquivo CSV chamado Planilha_Limpa_Filtrada_V3.2.csv na pasta do seu Drive. O parâmetro index=False impede que o Pandas adicione uma coluna extra de numeração de linhas no arquivo gerado.
